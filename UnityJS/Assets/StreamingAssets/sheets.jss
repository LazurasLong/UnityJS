...
if (gGoogleSheets) {
   ...
   ////////////////////////////////////////////////////////////////////////
    ...
    function MakeNamedRanges(sheetName)
    {
        var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
        if (!sheetName) {
            sheetName = "structSource";
        }
        var sheet = spreadsheet.getSheetByName(sheetName);
    ...
    function UpdateNamedRanges()
    {
        ...
        if (!rangesSheet) {
            ...
            rangesSheet = rangesSheet.getActiveSheet();
            spreadsheet.setActiveSheet(lastActiveSheet);
        }
        rangesSheet.clear();
        var topLeftRange = rangesSheet.getRange(1, 1, 1, 1);
        topLeftRange.setFormula('=ARRAYFORMULA(GenerateNamedRangeSheet(' + Math.random() + '))');
    ...
    function LoadSheet(sheets, sheetName)
    {
        ...
        var scope = SheetToScope(sheets, {}, 'rangeSchemas');
        if (scope.error) {
            console.log("sheets.js: LoadSheet: Error parsing sheet. Error in sheet name:", scope.errorScope.errorSheetName, "row:", scope.errorScope.errorRow, "column:", scope.errorScope.errorColumn, "error:", error, "errorScope:", scope.errorScope);
            return null;
        }
        return scope.value;
    }
    ...
    function LoadSheets(sheetSpecs, success, error)
    {
    ...
    function GetSheetURL(sheetSpec)
    {
        var sheetURL;
        ...
        if (params.live) {
            ...
            params.proxyPrefix = 'https://struct.studio/_p/miniProxy.php';
